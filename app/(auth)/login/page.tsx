...
export default function LoginPage() {
  ...
  async function signIn(event: FormEvent) {
    ...
    try {
      ...
      setAuthCookies(mockToken, role)
      setMessage('Notice: Logged in using Local Workspace Mode.')
      ...
      <form onSubmit={signIn} className="w-full max-w-md rounded-2xl bg-white p-8 shadow-xl shadow-slate-200">
        ...
        {!configured && (
          <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900">
            <strong>Local Mode Active:</strong> Standard form input enabled. Select workspace role below for instant access.
          </div>
        )}
        ...
        <div className="mt-2 flex items-center justify-between text-xs">
          <label className="flex items-center gap-2 text-slate-600">
            ...
            >
              ...
              <option value="panel">Expert Panel Reviewer</option>
        ...
        <p className="mt-6 text-center text-sm text-slate-500">
          New to the portal? <Link href="/signup" className="font-semibold text-blue-700 hover:underline">Create an account</Link>
        </p>
      </form>
    </main>
  )
