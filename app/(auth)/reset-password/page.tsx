...
export default function ResetPasswordPage() {
  ...
  async function handleUpdatePassword(event: FormEvent) {
    ...
    if (password !== confirmPassword) {
      setMessage(isRtl ? 'پاس ورڈز مماثل نہیں ہیں۔' : 'Passwords do not match.')
      return
    }
    ...
    try {
      ...
      setMessage(
        isRtl
          ? 'پاس ورڈ کامیابی سے اپ ڈیٹ ہو گیا! اب آپ اپنے نئے پاس ورڈ سے سائن ان کر سکتے ہیں۔'
      ...
      setMessage(
        error instanceof Error
          ? error.message
          : isRtl
          ? 'پاس ورڈ ری سیٹ کرنے میں ناکامی۔'
  ...
  return (
    <main className="relative grid min-h-screen place-items-center bg-slate-100 p-4 sm:p-6" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <form onSubmit={handleUpdatePassword} className="w-full max-w-md rounded-2xl bg-white p-6 sm:p-8 shadow-xl shadow-slate-200">
        <p className="eyebrow">{t('HEC ODL PORTAL')}</p>
        <h1 className="mt-2 text-2xl sm:text-3xl font-bold text-slate-900">{t('Reset Password')}</h1>
        <p className="mt-2 text-sm leading-6 text-slate-500">
          {isRtl
            ? 'اپنے ایچ ای سی او ڈی ایل پورٹل اکاؤنٹ کے لیے ایک مضبوط پاس ورڈ مقرر کریں۔'
        ...
        >
          {loading ? (isRtl ? 'پاس ورڈ تبدیل ہو رہا ہے…' : 'Updating password…') : t('Update password')}
        </button>
