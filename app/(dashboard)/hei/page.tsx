...
export default async function HeiDashboard() {
    ...
    try {
        ...
        // Fallback to 0 if database uninitialized during build
    }
    ...
    return (
        <PortalShell title="HEI workspace" subtitle="Track every ODL NOC application and action due from your institution.">
            ...
            <div className="mt-6">
                {totalCount === 0 ? (
                    <EmptyState title="No applications yet" text="Start with your institutional profile and then complete the controlled Model Application Dossier. Evidence is required for every Yes claim." />
                ) : (
                    <div className="rounded-xl border bg-white p-6 shadow-sm">
                        <h2 className="text-lg font-bold text-slate-900">Application Portfolio</h2>
                        <p className="mt-1 text-sm text-slate-500">Total active application dossiers registered in the system: <span className="font-semibold text-blue-700">{totalCount}</span></p>
                    </div>
                )}
                <div className="mt-6 text-center">
                    <Link href="/hei/applications/new" className="btn-primary">Start an ODL NOC application</Link>
                </div>
            </div>
        </PortalShell>
    )
