...
export default function Home() {
  return <main className="min-h-screen bg-slate-50 text-slate-900">
    ...
    <section className="border-y border-slate-800 bg-slate-900 text-white"><div className="mx-auto grid max-w-7xl gap-6 px-6 py-14 md:grid-cols-3"><Feature title="Controlled dossier" text="Parameter-wise claims, evidence, remarks and versioned submissions." /><Feature title="Role-based review" text="Dedicated HEI, QAD, Expert Panel and decision-maker workspaces." /><Feature title="AI, with safeguards" text="RAG policy support with local Ollama-first failover; rules remain deterministic." /></div></section>
  </main>
}
function Feature({ title, text }: { title: string; text: string }) {
  return (
    <div className="rounded-2xl border border-slate-800 bg-slate-800/90 p-6 shadow-xl hover:shadow-2xl hover:border-blue-400 transition-all duration-200">
