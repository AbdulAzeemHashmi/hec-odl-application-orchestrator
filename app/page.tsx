...
export default function Home() {
  return <main className="min-h-screen bg-slate-50 text-slate-900">
    ...
    <section className="border-y border-slate-300 bg-gradient-to-b from-slate-200 via-zinc-200 to-slate-200 text-slate-900"><div className="mx-auto grid max-w-7xl gap-6 px-6 py-14 md:grid-cols-3"><Feature title="Controlled dossier" text="Parameter-wise claims, evidence, remarks and versioned submissions." /><Feature title="Role-based review" text="Dedicated HEI, QAD, Expert Panel and decision-maker workspaces." /><Feature title="AI, with safeguards" text="RAG policy support with local Ollama-first failover; rules remain deterministic." /></div></section>
  </main>
}
function Feature({ title, text }: { title: string; text: string }) {
  return (
    <div className="rounded-2xl border border-slate-300/90 bg-white/95 p-6 shadow-md hover:shadow-xl hover:border-slate-400 transition-all duration-200">
