...
export class DeterministicClient implements AIClient {
    async invoke(prompt: string): Promise<string> {
        ...
        return cleanChatbotResponse(this.generatePolicyAnswer(lower))
    }
...
3. Faculty to Student Ratio: Institutions must maintain prescribed student teacher ratios (typically maximum 1:30 for tutoring cohorts) to ensure individual feedback.
4. Course Coordinators: Each ODL degree must designate a permanent PhD or Master's qualified Course Coordinator to oversee content delivery and continuous assessment.`
        }
...
1. Learning Management System (LMS): Institutions must operate a robust, 24/7 accessible LMS (such as Moodle, Canvas, or an equivalent institutional platform).
2. Features Required:
...
3. Professional Councils:
   Programs requiring accreditation (such as Computing, Business) must also fulfill the corresponding accreditation council's distance education standards.`
        }
...
1. Proctored Summative Exams: Final semester examinations must be conducted under strictly proctored conditions either physically at verified HEC or HEI study centers or via secure, monitored proctoring systems.
...
1. Institutional Eligibility: Universities must possess charter authority, accredited programs, and HEC certified institutional readiness before offering online or distance education.
2. Statutory Approvals: Academic Council and Syndicate or BoG approvals must be formally verified.
3. Technology Foundation: A 24/7 compliant Learning Management System (LMS) with secure proctoring and digital library access is required.
4. Faculty Readiness: Dedicated instructors must undergo specialized digital pedagogy and course development certifications.
5. Assessment Integrity: Strict summative proctoring and anti plagiarism controls must be enforced.
